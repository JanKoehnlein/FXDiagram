...
class XDiagramProvider {
	...
	def <T> createNode(T nodeObject, NodeMapping<T> nodeMapping, TransformationContext context) {
		if (nodeMapping.isApplicable(nodeObject)) {
			...
			nodeMapping.incoming.forEach[execute(nodeObject, context).forEach[target = node]]
			nodeMapping.outgoing.forEach[execute(nodeObject, context).forEach[source = node]]
			return node
		} else {
			return null
		}
	...
	protected def <T,U> List<XNode> execute(AbstractNodeMappingCall<T> nodeMappingCall, U domainArgument,
		...
		if (nodeMappingCall instanceof NodeMappingCall<?,?>) {
			...
			return #[createNode(nodeObject, nodeMappingCallCasted.nodeMapping, context)]
			...
			val nodeObjects = (nodeMappingCallCasted.selector as (Object)=>List<T>).apply(domainArgument)
			val result = newArrayList
			for (nodeObject : nodeObjects)
				result += createNode(nodeObject, nodeMappingCallCasted.nodeMapping, context)
			return result
		}
	}

	protected def <T,U> List<XConnection> execute(AbstractConnectionMappingCall<T> it, U domainArgument,
		TransformationContext context) {
		if (it instanceof ConnectionMappingCall<?,?>) {
			val connectionMappingCasted = it as ConnectionMappingCall<T,U>
			val connectionObject = (connectionMappingCasted.selector as (Object)=>T).apply(domainArgument)
			val connection = createConnection(connectionObject, connectionMappingCasted.connectionMapping, context)
			createEndpoints(connectionMappingCasted.connectionMapping, connectionObject, connection, context)
			return #[connection]
		} else if (it instanceof MultiConnectionMappingCall<?,?>) {
			val connectionMappingCasted = it as MultiConnectionMappingCall<T,U>
			val connectionObjects = (connectionMappingCasted.selector as (Object)=>List<T>).apply(domainArgument)
			val result = newArrayList
...
class TransformationContext {
	...
	def <T> getConnection(XtextDomainObjectDescriptor<T> descriptor) {
		diagram.connections.findFirst[domainObject == descriptor]
	}

	def <T> getNode(XtextDomainObjectDescriptor<T> descriptor) {
