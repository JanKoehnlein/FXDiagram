...
public class RadialMenu extends Pane {
	...
	public RadialMenu(final Options OPTIONS, final List<MenuItem> ITEMS) {
		...
		positions = Double.compare(degrees, 360.0) == 0 ? ITEMS.size() : ITEMS
				.size() - 1;
			...
			public void handle(final MouseEvent EVENT) {
				...
				if (MouseEvent.MOUSE_CLICKED == EVENT.getEventType()) {
					...
					if (SOURCE.equals(mainMenuMouseCatcher)) {
						...
						fireItemEvent(new ItemEvent(items.get(SOURCE), this,
								null, ItemEvent.ITEM_SELECTED));
						EVENT.consume();
					}
	...
	// final Point2D CENTER = new Point2D(getPrefWidth() * 0.5, getPrefHeight()
	// * 0.5);
	// final Circle MAIN_BUTTON = new Circle(CENTER.getX(), CENTER.getY(),
	// options.getButtonSize() * 0.5);
	// MAIN_BUTTON.setFill(new LinearGradient(0,
	// MAIN_BUTTON.getLayoutBounds().getMinY(),
	// 0, MAIN_BUTTON.getLayoutBounds().getMaxY(),
	// false, CycleMethod.NO_CYCLE,
	// new Stop(0.0, Color.hsb(options.getButtonInnerColor().getHue(), 0.80,
	// 0.90)),
	// new Stop(0.5, Color.hsb(options.getButtonInnerColor().getHue(), 0.75,
	// 0.90)),
	// new Stop(0.5, Color.hsb(options.getButtonInnerColor().getHue(), 0.85,
	// 0.90)),
	// new Stop(1.0, Color.hsb(options.getButtonInnerColor().getHue(), 0.90,
	// 0.80))));
	...
	// final Line HOR = new Line(CENTER.getX() - (0.1818181818 *
	// options.getButtonSize()), CENTER.getY(), CENTER.getX() + (0.1818181818 *
	// options.getButtonSize()), CENTER.getY());
	// final Line VER = new Line(CENTER.getX(), CENTER.getY() - (0.1818181818 *
	// options.getButtonSize()), CENTER.getX(), CENTER.getY() + (0.1818181818 *
	// options.getButtonSize()));
	...
	//
	// mainMenuMouseCatcher = new Circle(CENTER.getX(), CENTER.getY(),
	// options.getButtonSize() * 0.5);
	// mainMenuMouseCatcher.setFill(Color.TRANSPARENT);
	//
	// mainMenuMouseCatcher.addEventHandler(MouseEvent.MOUSE_CLICKED,
	// mouseHandler);
	// mainMenuMouseCatcher.addEventHandler(MouseEvent.MOUSE_PRESSED,
	// mouseHandler);
	// mainMenuMouseCatcher.addEventHandler(MouseEvent.MOUSE_RELEASED,
	// mouseHandler);
	// mainMenuMouseCatcher.addEventHandler(MouseEvent.MOUSE_ENTERED,
	// mouseHandler);
	//
	...
	//
	// button.setOpacity(options.isButtonVisible() ? options.getButtonAlpha() :
	// 0.0);
	//
	...
	private void initMenuItems(final List<MenuItem> ITEMS) {
		...
		for (int i = 0; i < ITEMS.size(); i++) {
			...
			if (Symbol.Type.NONE == item.getSymbol()
					&& item.getThumbnailImageName().isEmpty()) {
				...
				Canvas symbol = SymbolCanvas.getSymbol(item.getSymbol(),
						0.7 * item.getSize(), Color.WHITE);
				NODE.getChildren().add(symbol);
			}

			Circle itemMouseCatcher = new Circle(item.getSize() * 0.5);
			itemMouseCatcher.setFill(Color.TRANSPARENT);
			itemMouseCatcher.addEventFilter(MouseEvent.MOUSE_CLICKED,
					mouseHandler);
			...
			Point2D position = new Point2D(Math.cos(Math.toRadians(degree)),
	...
	private void registerListeners() {
		...
		heightProperty().addListener(new ChangeListener<Number>() {
			...
			public void changed(ObservableValue<? extends Number> ov,
				...
				if (oldHeight.doubleValue() != newHeight.doubleValue())
					isDirty = true;
			}
		});
	}
	...
	public void open() {
		...
		// rotate.play();
		openTimeLines[openTimeLines.length - 1]
				.setOnFinished(new EventHandler<ActionEvent>() {
					@Override
					public void handle(ActionEvent actionEvent) {
						fireMenuEvent(new MenuEvent(this, null,
								MenuEvent.MENU_OPEN_FINISHED));
					}
				});
	...
	public void close() {
		...
		// rotate.setInterpolator(Interpolator.EASE_BOTH);
		// rotate.play();
		closeTimeLines[closeTimeLines.length - 1]
				.setOnFinished(new EventHandler<ActionEvent>() {
					@Override
					public void handle(ActionEvent actionEvent) {
	...
	public void select(final MenuItem SELECTED_ITEM) {
		...
		for (Parent node : items.keySet()) {
			if (items.get(node).equals(SELECTED_ITEM)) {
				...
				ScaleTransition enlargeItem = new ScaleTransition(
				...
				ScaleTransition shrinkItem = new ScaleTransition(
						Duration.millis(300), node);
			...
			FadeTransition fadeOutItem = new FadeTransition(
...
//		if (options.isButtonHideOnSelect()) {
//			RotateTransition rotateMainButton = new RotateTransition(
//					Duration.millis(300), button);
//			rotateMainButton.setToAngle(225);
//			transitions.add(rotateMainButton);
//			FadeTransition fadeOutMainButton = new FadeTransition(
//					Duration.millis(300), button);
//			fadeOutMainButton.setToValue(0.0);
//			transitions.add(fadeOutMainButton);
//			ScaleTransition shrinkMainButton = new ScaleTransition(
//					Duration.millis(300), button);
...
//		}
	...
	private Canvas createCanvasThumbnail(final MenuItem ITEM) {
		...
		final double SIZE = THUMBNAIL.getWidth() > THUMBNAIL.getHeight() ? THUMBNAIL
				.getWidth() : THUMBNAIL.getHeight();
		final double SCALE = (0.7 * ITEM.getSize()) / SIZE;

		Canvas canvasThumbnail = new Canvas(0.7 * ITEM.getSize(),
				0.7 * ITEM.getSize());
	...
	private Timeline createItemOpenTimeLine(final StackPane NODE,
			final double X, final double Y, final double DELAY) {
		KeyValue kvX1 = new KeyValue(NODE.translateXProperty(), 0,
				Interpolator.EASE_OUT);
		KeyValue kvY1 = new KeyValue(NODE.translateYProperty(), 0,
				Interpolator.EASE_OUT);
		KeyValue kvR1 = new KeyValue(NODE.rotateProperty(), 0,
				Interpolator.EASE_OUT);
		KeyValue kvO1 = new KeyValue(NODE.opacityProperty(), 0,
				Interpolator.EASE_OUT);

		KeyValue kvX2 = new KeyValue(NODE.translateXProperty(), 0.0);
		KeyValue kvY2 = new KeyValue(NODE.translateYProperty(), 0.0);
		KeyValue kvSx2 = new KeyValue(NODE.scaleXProperty(), 1);
		KeyValue kvSy2 = new KeyValue(NODE.scaleYProperty(), 1);

		KeyValue kvX3 = new KeyValue(NODE.translateXProperty(), 1.1 * X,
				Interpolator.EASE_IN);
		KeyValue kvY3 = new KeyValue(NODE.translateYProperty(), 1.1 * Y,
				Interpolator.EASE_IN);

		KeyValue kvX4 = new KeyValue(NODE.translateXProperty(), 0.95 * X,
				Interpolator.EASE_OUT);
		KeyValue kvY4 = new KeyValue(NODE.translateYProperty(), 0.95 * Y,
				Interpolator.EASE_OUT);
		KeyValue kvRO4 = new KeyValue(NODE.rotateProperty(), 360);
		KeyValue kvO4 = new KeyValue(NODE.opacityProperty(), 1.0,
				Interpolator.EASE_OUT);
		...
		KeyFrame kfO2 = new KeyFrame(Duration.millis(50 + DELAY), kvX2, kvY2, kvSx2, kvSy2);
		KeyFrame kfO3 = new KeyFrame(Duration.millis(250 + DELAY), kvX3, kvY3);
		KeyFrame kfO4 = new KeyFrame(Duration.millis(400 + DELAY), kvX4, kvY4,
				kvRO4, kvO4);
	...
	private Timeline createItemCloseTimeLine(final StackPane NODE,
			final double X, final double Y, final double DELAY) {
		KeyValue kvX1 = new KeyValue(NODE.translateXProperty(), 0,
				Interpolator.EASE_OUT);
		KeyValue kvY1 = new KeyValue(NODE.translateYProperty(), 0,
		...
		KeyFrame kfC2 = new KeyFrame(Duration.millis(50 + DELAY), kvX4, kvY4,
				kvRC4, kvO4);
		...
		KeyFrame kfC5 = new KeyFrame(Duration.millis(600 + DELAY), kvX1, kvY1,
				kvR1, kvO1);
	...
	private void resize() {
		...
		for (Parent node : items.keySet()) {
			...
			node.setLayoutY((getPrefHeight() - node.getLayoutBounds()
					.getHeight()) * 0.5);
		}
	}
	...
	public static class ItemEvent extends Event {
		public static final EventType<ItemEvent> ITEM_SELECTED = new EventType(
				ANY, "itemSelected");

		private MenuItem item;

		// ******************** Constructors **********************************
		public ItemEvent(final MenuItem ITEM, final Object SOURCE,
				final EventTarget TARGET, final EventType<ItemEvent> EVENT_TYPE) {
	...
	public static class MenuEvent extends Event {
		public static final EventType<MenuEvent> MENU_OPEN_STARTED = new EventType(
				ANY, "menuOpenStarted");
		public static final EventType<MenuEvent> MENU_OPEN_FINISHED = new EventType(
				ANY, "menuOpenFinished");
		public static final EventType<MenuEvent> MENU_CLOSE_STARTED = new EventType(
				ANY, "menuCloseStarted");
		public static final EventType<MenuEvent> MENU_CLOSE_FINISHED = new EventType(
				ANY, "menuCloseFinished");

		// ******************** Constructors **********************************
		public MenuEvent(final Object SOURCE, final EventTarget TARGET,
				final EventType<MenuEvent> EVENT_TYPE) {
			super(SOURCE, TARGET, EVENT_TYPE);
		}
	}
}
